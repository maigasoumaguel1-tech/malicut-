...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ('/','/index.html'):
            ...
            self._send_file('favicon.png', 'image/png')
        elif path == '/icon-180.png':
            # Accepte le nom normal et le nom accentué créé par certains téléphones/GitHub.
            if os.path.exists('icon-180.png'):
                self._send_file('icon-180.png', 'image/png')
            else:
                self._send_file('icône-180.png', 'image/png')
        elif path == '/icon-192.png':
            if os.path.exists('icon-192.png'):
                self._send_file('icon-192.png', 'image/png')
            else:
                self._send_file('icône-192.png', 'image/png')
